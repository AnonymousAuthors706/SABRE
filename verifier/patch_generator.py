...
def find_patch_variable_ARM(cfg, node_addr, expl_instr_addr, exp_func, tgt):
    ...
    # print("======== starting find_patch_variable_ARM() =============")
    ...
    while cfg.arch.sp not in base_reg or first:
        ...
        while i >= 0 and cfg.arch.sp not in base_reg:
            ...
            # if 'add.w' in instr.instr:
                # print(f"\t{def_via_other} = {base_reg} {len(base_reg)} in {instr.arg.split(', ')[0]} {len(instr.arg.split(', ')[0])}")
            if len(parts) > 2:
                # print(f"parts = {parts}")
                # print(f"'{base_reg}' in '{parts[1]}' --> {base_reg in parts[1]}")
                # print(f"'{param}' in '{parts[2]}' --> {param in parts[2]}")
            ...
            if def_via_other:
                ...
                if def_via_str:
                    ...
                    # print(f"\tbase_reg = {base_reg} {len(base_reg)}")
                    # print(f"\tdef_addr = {def_addr}")
...
def generate_patch_ARM(cfg, node_addr, expl_instr_addr, loopcount, exp_func, cflog, cflog_idx, vul_mem_func_bounds):
    start = time.time()
    ...
    ### printing
    # print(f'Generated Patches: {len(pg.patches.keys())}')
    # for p in pg.patches.keys():
        # print(f"{p}\n{pg.patches[p]}\n")
